...
std::string metrics::measurement::to_string() {
	...
	auto sunits = (written >= ONE_MB) ? "MB"
		: (written >= ONE_KB) ? "KB"
		: "B";
	auto sspace = (written >= ONE_KB) ? "" : " ";
	auto sdivisor = (written >= ONE_MB) ? ONE_MB
		: (written >= ONE_KB) ? ONE_KB
		: ONE_B;
...
metrics::metrics()
	: connections_(0), read_(0), written_(0)
	, last_measurement_(), last_measured_at_(std::chrono::steady_clock::now())
{}
